		...
		if(strlen(f) > 0 && f[0] != '#') {
			...
			if((arg = strchr(f, ' ')) != NULL) {
				...
				if(strcmp(f, "usemtl") == 0) {
					...
					if(c == 3) GSMathNormalTriangle(face.normal, face.vertex[0], face.vertex[1], face.vertex[2]);
					if(c == 4) GSMathNormalQuad(face.normal, face.vertex[0], face.vertex[1], face.vertex[2], face.vertex[3]);
