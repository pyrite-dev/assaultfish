 ...
 */
int jar_xm_create_context_from_file(jar_xm_context_t** ctx, GSU32 rate, const char* filename);
 ...
 * @see jar_xm_create_context_safe()
 */
int jar_xm_create_context(jar_xm_context_t**, const char* moddata, GSU32 rate);
 ...
 * @returns 2 if memory allocation failed
 */
int jar_xm_create_context_safe(jar_xm_context_t**, const char* moddata, size_t moddata_length, GSU32 rate);
 ...
 * @param numsamples number of samples to generate
 */
void jar_xm_generate_samples_16bit(jar_xm_context_t* ctx, GSI16* output, size_t numsamples);
 ...
 * @param numsamples number of samples to generate
 */
void jar_xm_generate_samples_8bit(jar_xm_context_t* ctx, GSI8* output, size_t numsamples);
 ...
 * indefinitely. */
void jar_xm_set_max_loop_count(jar_xm_context_t*, GSU8 loopcnt);
 ...
 * once, etc. */
GSU8 jar_xm_get_loop_count(jar_xm_context_t*);
 ...
 */
GSBool jar_xm_mute_channel(jar_xm_context_t*, GSU16, GSBool);
 ...
 */
GSBool jar_xm_mute_instrument(jar_xm_context_t*, GSU16, GSBool);
...
GSU16 jar_xm_get_number_of_channels(jar_xm_context_t*);

/** Get the module length (in patterns). */
GSU16 jar_xm_get_module_length(jar_xm_context_t*);

/** Get the number of patterns. */
GSU16 jar_xm_get_number_of_patterns(jar_xm_context_t*);
 ...
 */
GSU16 jar_xm_get_number_of_rows(jar_xm_context_t*, GSU16);

/** Get the number of instruments. */
GSU16 jar_xm_get_number_of_instruments(jar_xm_context_t*);
 ...
 * jar_xm_get_number_of_instruments(...).
 */
GSU16 jar_xm_get_number_of_samples(jar_xm_context_t*, GSU16);
 ...
 */
void jar_xm_get_playing_speed(jar_xm_context_t*, GSU16* bpm, GSU16* tempo);
 ...
 * generated audio)
 */
void jar_xm_get_position(jar_xm_context_t*, GSU8* pattern_index, GSU8* pattern, GSU8* row, GSU64* samples);
 ...
 * jar_xm_get_number_of_instruments(...).
 */
GSU64 jar_xm_get_latest_trigger_of_instrument(jar_xm_context_t*, GSU16);
 ...
 */
GSU64 jar_xm_get_latest_trigger_of_sample(jar_xm_context_t*, GSU16 instr, GSU16 sample);
 ...
 * @note Channel numbers go from 1 to jar_xm_get_number_of_channels(...).
 */
GSU64 jar_xm_get_latest_trigger_of_channel(jar_xm_context_t*, GSU16);
 ...
 */
GSU64 jar_xm_get_remaining_samples(jar_xm_context_t*);
...
struct jar_xm_envelope_s {
	...
	GSU8		num_points;
	GSU8		sustain_point;
	GSU8		loop_start_point;
	GSU8		loop_end_point;
	GSBool		enabled;
	GSBool		sustain_enabled;
	GSBool		loop_enabled;
};
...
struct jar_xm_instrument_s {
	...
	GSU16	       num_samples;
	GSU8	       sample_of_notes[NUM_NOTES];
...
struct jar_xm_pattern_slot_s {
	GSU8 note;       /* 1-96, 97 = Key Off note */
	GSU8 instrument; /* 1-128 */
	GSU8 volume_column;
	GSU8 effect_type;
	GSU8 effect_param;
};
...
struct jar_xm_module_s {
	...
	GSU16		length;
	GSU16		restart_position;
	GSU16		num_channels;
	GSU16		num_patterns;
	GSU16		num_instruments;
	jar_xm_frequency_type_t frequency_type;
	GSU8		pattern_table[PATTERN_ORDER_TABLE_LENGTH];
...
struct jar_xm_channel_context_s {
	...
	float	  frequency;
	float	  step;
	GSBool ping; /* For ping-pong samples: GSTrue is -->, GSFalse is <-- */

	float volume;  /* Ideally between 0 (muted) and 1 (loudest) */
	float panning; /* Between 0 (left) and 1 (right); 0.5 is centered */

	GSU16 autovibrato_ticks;

	GSBool   sustained;
	float	    fadeout_volume;
	float	    volume_envelope_volume;
	float	    panning_envelope_panning;
	GSU16 volume_envelope_frame_count;
	GSU16 panning_envelope_frame_count;

	float autovibrato_note_offset;

	GSBool	       arp_in_progress;
	GSU8	       arp_note_offset;
	GSU8	       volume_slide_param;
	GSU8	       fine_volume_slide_param;
	GSU8	       global_volume_slide_param;
	GSU8	       panning_slide_param;
	GSU8	       portamento_up_param;
	GSU8	       portamento_down_param;
	GSU8	       fine_portamento_up_param;
	GSU8	       fine_portamento_down_param;
	GSU8	       extra_fine_portamento_up_param;
	GSU8	       extra_fine_portamento_down_param;
	GSU8	       tone_portamento_param;
	float		       tone_portamento_target_period;
	GSU8	       multi_retrig_param;
	GSU8	       note_delay_param;
	GSU8	       pattern_loop_origin; /* Where to restart a E6y loop */
	GSU8	       pattern_loop_count;  /* How many loop passes have been done */
	GSBool	       vibrato_in_progress;
	jar_xm_waveform_type_t vibrato_waveform;
	GSBool	       vibrato_waveform_retrigger; /* True if a new note retriggers the waveform */
	GSU8	       vibrato_param;
	GSU16	       vibrato_ticks; /* Position in the waveform */
	float		       vibrato_note_offset;
	jar_xm_waveform_type_t tremolo_waveform;
	GSBool	       tremolo_waveform_retrigger;
	GSU8	       tremolo_param;
	GSU8	       tremolo_ticks;
...
struct jar_xm_context_s {
	...
	GSU8  current_table_index;
	GSU8  current_row;
	GSU16 current_tick; /* Can go below 255, with high tempo and a pattern delay */
	float	    remaining_samples_in_tick;
	GSU64 generated_samples;

	GSBool  position_jump;
	GSBool  pattern_break;
	GSU8 jump_dest;
	GSU8 jump_row;
	...
	GSU8* row_loop_count; /* Array of size MAX_NUM_ROWS * module_length */
	GSU8  loop_count;
	GSU8  max_loop_count;
...
int jar_xm_create_context(jar_xm_context_t** ctxp, const char* moddata, GSU32 rate) { return jar_xm_create_context_safe(ctxp, moddata, SIZE_MAX, rate); }

int jar_xm_create_context_safe(jar_xm_context_t** ctxp, const char* moddata, size_t moddata_length, GSU32 rate) {
#if JAR_XM_DEFENSIVE
	int ret;
#endif
	size_t		  bytes_needed;
	...
	if((ret = jar_xm_check_sanity_preload(moddata, moddata_length))) {
		JX_DEBUG("jar_xm_check_sanity_preload() returned %i, module is not safe to load", ret);
		return 1;
	}
#endif
	...
	if(mempool == NULL && bytes_needed > 0) {
		...
		JX_DEBUG("call to malloc() failed, returned %p", (void*)mempool);
		return 2;
	}

	/* Initialize most of the fields to 0, 0.f, NULL or GSFalse depending on type */
	...
	for(i = 0; i < ctx->module.num_channels; ++i) {
		...
		ch->vibrato_waveform_retrigger = GSTrue;
		ch->tremolo_waveform	       = jar_xm_SINE_WAVEFORM;
		ch->tremolo_waveform_retrigger = GSTrue;
	...
	ctx->row_loop_count = (GSU8*)mempool;
	mempool += MAX_NUM_ROWS * sizeof(GSU8);

#if JAR_XM_DEFENSIVE
	if((ret = jar_xm_check_sanity_postload(ctx))) {
		JX_DEBUG("jar_xm_check_sanity_postload() returned %i, module is not safe to play", ret);
...
void jar_xm_free_context(jar_xm_context_t* context) { free(context->allocated_memory); }

void jar_xm_set_max_loop_count(jar_xm_context_t* context, GSU8 loopcnt) { context->max_loop_count = loopcnt; }

GSU8 jar_xm_get_loop_count(jar_xm_context_t* context) { return context->loop_count; }

int jar_xm_get_last(jar_xm_context_t* ctx) { return ctx->last; }

GSBool jar_xm_mute_channel(jar_xm_context_t* ctx, GSU16 channel, GSBool mute) {
	GSBool old			 = ctx->channels[channel - 1].muted;
	ctx->channels[channel - 1].muted = mute;
	return old;
}

GSBool jar_xm_mute_instrument(jar_xm_context_t* ctx, GSU16 instr, GSBool mute) {
	GSBool old				 = ctx->module.instruments[instr - 1].muted;
	ctx->module.instruments[instr - 1].muted = mute;
	return old;
}

const char* jar_xm_get_module_name(jar_xm_context_t* ctx) { return ctx->module.name; }

const char* jar_xm_get_tracker_name(jar_xm_context_t* ctx) { return ctx->module.trackername; }

GSU16 jar_xm_get_number_of_channels(jar_xm_context_t* ctx) { return ctx->module.num_channels; }

GSU16 jar_xm_get_module_length(jar_xm_context_t* ctx) { return ctx->module.length; }

GSU16 jar_xm_get_number_of_patterns(jar_xm_context_t* ctx) { return ctx->module.num_patterns; }

GSU16 jar_xm_get_number_of_rows(jar_xm_context_t* ctx, GSU16 pattern) { return ctx->module.patterns[pattern].num_rows; }

GSU16 jar_xm_get_number_of_instruments(jar_xm_context_t* ctx) { return ctx->module.num_instruments; }

GSU16 jar_xm_get_number_of_samples(jar_xm_context_t* ctx, GSU16 instrument) { return ctx->module.instruments[instrument - 1].num_samples; }

void jar_xm_get_playing_speed(jar_xm_context_t* ctx, GSU16* bpm, GSU16* tempo) {
	if(bpm) *bpm = ctx->bpm;
	if(tempo) *tempo = ctx->tempo;
}

void jar_xm_get_position(jar_xm_context_t* ctx, GSU8* pattern_index, GSU8* pattern, GSU8* row, GSU64* samples) {
...
GSU64 jar_xm_get_latest_trigger_of_instrument(jar_xm_context_t* ctx, GSU16 instr) { return ctx->module.instruments[instr - 1].latest_trigger; }

GSU64 jar_xm_get_latest_trigger_of_sample(jar_xm_context_t* ctx, GSU16 instr, GSU16 sample) { return ctx->module.instruments[instr - 1].samples[sample].latest_trigger; }

GSU64 jar_xm_get_latest_trigger_of_channel(jar_xm_context_t* ctx, GSU16 chn) { return ctx->channels[chn - 1].latest_trigger; }
 ...
 */
#define READ_U8(offset) (((offset) < moddata_length) ? (*(GSU8*)(moddata + (offset))) : 0)
#define READ_U16(offset) ((GSU16)READ_U8(offset) | ((GSU16)READ_U8((offset) + 1) << 8))
#define READ_U32(offset) ((GSU32)READ_U16(offset) | ((GSU32)READ_U16((offset) + 2) << 16))
...
int jar_xm_check_sanity_postload(jar_xm_context_t* ctx) {
	...
	/* Check the POT */
	for(i = 0; i < ctx->module.length; ++i) {
		if(ctx->module.pattern_table[i] >= ctx->module.num_patterns) {
			if(i + 1 == ctx->module.length && ctx->module.length > 1) {
				/* Cheap fix */
				--ctx->module.length;
				JX_DEBUG("trimming invalid POT at pos %X", i);
			} else {
				JX_DEBUG("module has invalid POT, pos %X references nonexistent pattern %X", i, ctx->module.pattern_table[i]);
				return 1;
			}
		}
	}
...
size_t jar_xm_get_memory_needed_for_context(const char* moddata, size_t moddata_length) {
	...
	memory_needed += MAX_NUM_ROWS * READ_U16(offset + 4) * sizeof(GSU8); /* Module length */
	...
	for(i = 0; i < num_instruments; ++i) {
		GSU16 num_samples;
		GSU32 sample_header_size	  = 0;
		GSU32 sample_size_aggregate = 0;
		GSU16 j;
...
char* jar_xm_load_module(jar_xm_context_t* ctx, const char* moddata, size_t moddata_length, char* mempool) {
	...
	for(i = 0; i < mod->num_patterns; ++i) {
		GSU16	  packed_patterndata_size = READ_U16(offset + 7);
	...
	for(i = 0; i < ctx->module.num_instruments; ++i) {
		...
		for(j = 0; j < instr->num_samples; ++j) {
			...
			sample->finetune    = (GSI8)READ_U8(offset + 13);
			...
			sample->relative_note = (GSI8)READ_U8(offset + 16);
		...
		for(j = 0; j < instr->num_samples; ++j) {
			/* Read sample data */
			jar_xm_sample_t* sample = instr->samples + j;
			GSU32	 length = sample->length;
			GSU32	 k;

			if(sample->bits == 16) {
				GSI16 v = 0;
				for(k = 0; k < length; ++k) {
					v		= v + (GSI16)READ_U16(offset + (k << 1));
...
static float jar_xm_waveform(jar_xm_waveform_type_t, GSU8);
static void  jar_xm_autovibrato(jar_xm_context_t*, jar_xm_channel_context_t*);
static void  jar_xm_vibrato(jar_xm_context_t*, jar_xm_channel_context_t*, GSU8, GSU16);
static void  jar_xm_tremolo(jar_xm_context_t*, jar_xm_channel_context_t*, GSU8, GSU16);
static void  jar_xm_arpeggio(jar_xm_context_t*, jar_xm_channel_context_t*, GSU8, GSU16);
...
static void  jar_xm_panning_slide(jar_xm_channel_context_t*, GSU8);
static void  jar_xm_volume_slide(jar_xm_channel_context_t*, GSU8);

static float jar_xm_envelope_lerp(jar_xm_envelope_point_t*, jar_xm_envelope_point_t*, GSU16);
static void  jar_xm_envelope_tick(jar_xm_channel_context_t*, jar_xm_envelope_t*, GSU16*, float*);
...
static const GSU16 amiga_frequencies[] = {
...
static float jar_xm_waveform(jar_xm_waveform_type_t waveform, GSU8 step) {
...
static void jar_xm_vibrato(jar_xm_context_t* ctx, jar_xm_channel_context_t* ch, GSU8 param, GSU16 pos) {
...
static void jar_xm_tremolo(jar_xm_context_t* ctx, jar_xm_channel_context_t* ch, GSU8 param, GSU16 pos) {
	unsigned int step = pos * (param >> 4);
...
static void jar_xm_arpeggio(jar_xm_context_t* ctx, jar_xm_channel_context_t* ch, GSU8 param, GSU16 tick) {
	switch(tick % 3) {
	case 0:
		ch->arp_in_progress = GSFalse;
		ch->arp_note_offset = 0;
		break;
	case 2:
		ch->arp_in_progress = GSTrue;
...
static void jar_xm_panning_slide(jar_xm_channel_context_t* ch, GSU8 rawval) {
	float f;
...
static void jar_xm_volume_slide(jar_xm_channel_context_t* ch, GSU8 rawval) {
	float f;
...
static float jar_xm_envelope_lerp(jar_xm_envelope_point_t* a, jar_xm_envelope_point_t* b, GSU16 pos) {
...
static float jar_xm_linear_period(float note) { return 7680.f - note * 64.f; }

static float jar_xm_linear_frequency(float period) { return 8363.f * powf(2.f, (4608.f - period) / 768.f); }

static float jar_xm_amiga_period(float note) {
	unsigned int intnote = note;
	GSU8   a	     = intnote % 12;
	GSI8    octave  = note / 12.f - 2;
	GSU16  p1 = amiga_frequencies[a], p2 = amiga_frequencies[a + 1];
...
static float jar_xm_frequency(jar_xm_context_t* ctx, float period, float note_offset) {
	...
	case jar_xm_AMIGA_FREQUENCIES:
		...
		if(JAR_XM_DEBUG && (p1 < period || p2 > period)) {
			JX_DEBUG("%i <= %f <= %i should hold but doesn't, this is a bug", p2, period, p1);
		}
...
static void jar_xm_handle_note_and_instrument(jar_xm_context_t* ctx, jar_xm_channel_context_t* ch, jar_xm_pattern_slot_t* s) {
	...
	case 9: /* 9xx: Sample offset */
		if(ch->sample != NULL && NOTE_IS_VALID(s->note)) {
			GSU32 final_offset = s->effect_param << (ch->sample->bits == 16 ? 7 : 8);
...
static void jar_xm_row(jar_xm_context_t* ctx) {
	...
	if(ctx->position_jump) {
		...
		ctx->position_jump	 = GSFalse;
		ctx->pattern_break	 = GSFalse;
		ctx->jump_row		 = 0;
...
static void jar_xm_envelope_tick(jar_xm_channel_context_t* ch, jar_xm_envelope_t* env, GSU16* counter, float* outval) {
	if(env->num_points < 2) {
		...
		if(env->loop_enabled) {
			GSU16 loop_start	= env->points[env->loop_start_point].frame;
			GSU16 loop_end	= env->points[env->loop_end_point].frame;
			GSU16 loop_length = loop_end - loop_start;
...
static void jar_xm_tick(jar_xm_context_t* ctx) {
	...
	for(i = 0; i < ctx->module.num_channels; ++i) {
		...
		if(ch->vibrato_in_progress && !HAS_VIBRATO(ch->current)) {
			ch->vibrato_in_progress = GSFalse;
			ch->vibrato_note_offset = 0.f;
			jar_xm_update_frequency(ctx, ch);
		}
		...
		case 17: /* Hxy: Global volume slide */
			if(ctx->current_tick == 0) break;
			if((ch->global_volume_slide_param & 0xF0) && (ch->global_volume_slide_param & 0x0F)) {
				/* Illegal state */
				break;
			}
...
static float jar_xm_next_of_sample(jar_xm_channel_context_t* ch) {
	...
	a = (GSU32)ch->sample_position; /* This cast is fine,
...
static void jar_xm_sample(jar_xm_context_t* ctx, size_t snum, float* left, float* right) {
	...
	*right *= fgvol;
	...
	if(fabs(*left) > 1 || fabs(*right) > 1) {
		JX_DEBUG("clipping frame: %f %f, this is a bad module or a libxm bug", *left, *right);
	}
#endif
}
...
GSU64 jar_xm_get_remaining_samples(jar_xm_context_t* ctx) {
	GSU64 total	     = 0;
	GSU8  currentLoopCount = jar_xm_get_loop_count(ctx);
...
int jar_xm_create_context_from_file(jar_xm_context_t** ctx, GSU32 rate, const char* filename) {
	...
	if(xmf == NULL) {
		JX_DEBUG_ERR("Could not open input file");
		*ctx = NULL;
		return 3;
	}
	...
	case 1:
		JX_DEBUG("could not create context: module is not sane\n");
		*ctx = NULL;
		return 1;
		break;

	case 2:
		JX_FATAL("could not create context: malloc failed\n");
		return 2;
		break;

	default:
		JX_FATAL("could not create context: unknown error\n");
		return 6;
		break;
	}
