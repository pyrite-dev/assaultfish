...
#if 0
/**
 * This is the original definitions, but not preferred
 * since these type size might differ.
 */
...
GSBool jar_mod_init(jar_mod_context_t* modctx);
GSBool jar_mod_setcfg(jar_mod_context_t* modctx, int samplerate, int bits, int stereo, int stereo_separation, int filter);
void	  jar_mod_fillbuffer(jar_mod_context_t* modctx, GSI16* outbuffer, unsigned long nbsample, jar_mod_tracker_buffer_state* trkbuf);
void	  jar_mod_unload(jar_mod_context_t* modctx);
mulong	  jar_mod_load_file(jar_mod_context_t* modctx, const char* filename);
GSBool jar_mod_load(jar_mod_context_t* modctx, void* mod_data, int mod_data_size);
...
static const short sintable[] = {0, 24, 49, 74, 97, 120, 141, 161, 180, 197, 212, 224, 235, 244, 250, 253, 255, 253, 250, 244, 235, 224, 212, 197, 180, 161, 141, 120, 97, 74, 49, 24};
...
modtype modlist[] = {{"M!K!", 4}, {"M.K.", 4}, {"FLT4", 4}, {"FLT8", 8}, {"4CHN", 4}, {"6CHN", 6}, {"8CHN", 8}, {"10CH", 10}, {"12CH", 12}, {"14CH", 14}, {"16CH", 16}, {"18CH", 18}, {"20CH", 20}, {"22CH", 22}, {"24CH", 24}, {"26CH", 26}, {"28CH", 28}, {"30CH", 30}, {"32CH", 32}, {"", 0}};
...
static void worknote(note* nptr, channel* cptr, char t, jar_mod_context_t* mod) {
	...
	case EFFECT_ARPEGGIO:
		...
		*/
		...
		if(effect & 0xff) {
			...
			if(arpnote >= FULL_PERIOD_TABLE_LENGTH) arpnote = FULL_PERIOD_TABLE_LENGTH - 1;
	...
	case EFFECT_VOLSLIDE_TONEPORTA:
		...
		*/
		...
		if((effect & 0xFF) != 0) cptr->volumeslide = (effect & 0xFF);
	...
	case EFFECT_EXTENDED:
		...
		case EFFECT_E_FINE_PORTA_UP:
			...
			*/
			...
			if(cptr->period < 113) cptr->period = 113;
			break;
		...
		case EFFECT_E_FINE_PORTA_DOWN:
			...
			*/
			...
			cptr->period += (effect & 0xF);
			if(cptr->period > 856) cptr->period = 856;
			break;
		...
		case EFFECT_E_FINE_VOLSLIDE_DOWN:
			...
			*/
			...
			cptr->volume -= (effect & 0xF);
			if(cptr->volume > 200) cptr->volume = 0;
			break;
		...
		case EFFECT_E_NOTE_CUT:
			...
			*/
			...
			if(!cptr->cut_param) cptr->volume = 0;
			break;
...
static void workeffect(note* nptr, channel* cptr) {
	...
	case EFFECT_ARPEGGIO:
		...
		if(cptr->parameffect) {
			...
			cptr->ArpIndex++;
			if(cptr->ArpIndex > 2) cptr->ArpIndex = 0;
		}
		break;
	...
	case EFFECT_PORTAMENTO_UP:
		...
		if(cptr->period) {
			...
			if(cptr->period < 113 || cptr->period > 20000) cptr->period = 113;
		}
	...
	case EFFECT_PORTAMENTO_DOWN:
		...
		if(cptr->period) {
			...
			if(cptr->period > 20000) cptr->period = 20000;
		}
	...
	case EFFECT_TONE_PORTAMENTO:
		...
		if(cptr->effect == EFFECT_VOLSLIDE_TONEPORTA) {
			if(cptr->volumeslide > 0x0F) {
				...
				if(cptr->volume > 63) cptr->volume = 63;
			} else {
	...
	case EFFECT_VIBRATO:
		...
		if(cptr->effect == EFFECT_VOLSLIDE_VIBRATO) {
			if(cptr->volumeslide > 0xF) {
				...
				if(cptr->volume > 64) cptr->volume = 0;
			}
		}
	...
	case EFFECT_E_NOTE_CUT:
		if(cptr->cut_param) cptr->cut_param--;
...
GSBool jar_mod_setcfg(jar_mod_context_t* modctx, int samplerate, int bits, int stereo, int stereo_separation, int filter) {
	if(modctx) {
		...
		if(bits == 8 || bits == 16) modctx->bits = bits;
		else
			modctx->bits = 16;

		if(filter) modctx->filter = 1;
		else
			modctx->filter = 0;
...
GSBool jar_mod_load(jar_mod_context_t* modctx, void* mod_data, int mod_data_size) {
	...
	if(modmemory) {
		if(modctx) {
			...
			if(modmemory >= endmodmemory) return 0; /* End passed ? - Probably a bad file ! */
			...
			for(i = 0; i < 31; i++) modctx->sampledata[i] = 0;
			...
			for(i = 0; i < nbsample; i++) {
				...
				if(modctx->patternticks++ > modctx->patternticksaim) {
					if(!modctx->patterndelay) {
						...
						if(!modctx->jump_loop_effect) modctx->patternpos += modctx->number_of_channels;
						else
					...
					if(cptr->period != 0) {
						...
						if(cptr->replen <= 2) {
							if((cptr->samppos >> 10) >= (cptr->length)) {
								...
								if(cptr->length) cptr->samppos = cptr->samppos % (((unsigned long)cptr->length) << 10);
								else
									cptr->samppos = 0;
							}
						} else {
				...
				if(trkbuf && !state_remaining_steps) {
					...
					if(trkbuf->nb_of_state < trkbuf->nb_max_of_state) trkbuf->nb_of_state++;
				} else {
...
static GSBool jar_mod_reset(jar_mod_context_t* modctx) {
	if(modctx) {
...
mulong jar_mod_current_samples(jar_mod_context_t* modctx) {
	if(modctx) return modctx->samplenb;
